...
export const ProgramProvider: FC<ProgramProviderProps> = ({children}) => {
        ...
        try{
            if(wallet.address===undefined){
                ...
                if(stakeData?.content?.dataType==="moveObject"){
                    ...
                    let uD = stakeData.content.fields as any
                    let poolData = (await getStakingPoolData()) as any
                    if(poolData!=null)
    ...
    const getUserCoinflipData = async(selectedCoin: number) => {
        try{
            if(wallet.address===undefined) return null
            let coinflipData = (await provider.getDynamicFieldObject({parentId: InfoCoinflip[selectedCoin].pool, name: {type:"address", value: wallet.address}})).data
    ...
    const coinflip_flip = useCallback(async(selectedCoin: number, selectedSide: number, selectedAmount: number)=>{
        let coins = (await provider.getCoins({
            owner: wallet.address!,
            coinType: InfoCoinflip[selectedCoin].token_type
        })).data
        let amount = selectedAmount * (10 ** InfoCoinflip[selectedCoin].token_decimals)
        if(coins.length===0) throw new Error("No token");
        let total=0;
        for(let item of coins) total+=Number(item.balance)
        ...
        const tx = new TransactionBlock()
        if(InfoCoinflip[selectedCoin].token_type==='0x02::sui::SUI'){
            if(coins.length>1){
                tx.mergeCoins(tx.gas, coins.slice(1, coins.length).map(item=>{return tx.object(item.coinObjectId)}))
            }
            tx.moveCall({
                target: `${InfoCoinflip[selectedCoin].contract}::coinflip::play_mut`,
                typeArguments:[
                    InfoCoinflip[selectedCoin].token_type
                ],
                arguments:[
                    tx.object(InfoCoinflip[selectedCoin].pool),
                    tx.gas,
                    tx.pure(amount),
    ...
    const coinflip_claim = useCallback(async(selectedCoin: number)=>{
        const tx = new TransactionBlock()
        tx.moveCall({
            target: `${InfoCoinflip[selectedCoin].contract}::coinflip::claim`,
            typeArguments:[
                InfoCoinflip[selectedCoin].token_type
            ],
            arguments:[
